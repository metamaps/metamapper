...
function collectWhenTopics (context, config, cb) {
  ...
  rtmBot.send(iT('en.buildingContext.collectFocalTopic.explainHasTopics'), facilitatorDM)
  const formatted = topics.map(t => {
    const metacode = Metamaps.findMetacodeByNameIdOrEmoji(t.metacode_id)
    return `(${t.id}) :${metacode[2]}: ${t.name}\n`
  }).join('')
  rtmBot.send(formatted, facilitatorDM)
...
function collectWhenNoTopics (context, config, cb) {
  ...
  rtmBot.send(iT('en.buildingContext.collectFocalTopic.explainNoTopics'), facilitatorDM)
  listenInChannel(rtmBot, facilitatorDM, function (err, message) {
    if (err) {
      cb(err)
      return
...
function collectMetacode (context, config, cb) {
  ...
  rtmBot.send(iT('en.buildingContext.collectMetacode.explain'), facilitatorDM)
  rtmBot.send(metacodes.join(' '), facilitatorDM)
  ...
  function collect () {
    listenInChannel(rtmBot, facilitatorDM, function (err, message) {
      ...
      if (metacode) {
        ...
        rtmBot.send(iT('en.buildingContext.collectMetacode.tryAgain'), facilitatorDM)
        collect()
      }
    })
  }
...
function main (context, configuration, cb) {
  ...
  function handleParticipantMessage (userId, dmId, message) {
    ...
    addTopicToMap(id, topic, tokens[userId], function (err, t) {
      if (err) {
        rtmBot.send('There was an error creating that topic', err)
        return
      }
      rtmBot.send(iT('en.buildingContext.createdTopic'), dmId)
  ...
  function setTopic (idOrText) {
    ...
    collectMetacode(context, configuration, function (err, metacode) {
      if (err) {
        rtmBot.send(iT('en.buildingContext.facilitatorSetTopicError'), facilitatorDM)
        return
      }
      function complete(topic) {
        focalTopic = topic
        selectedMetacode = metacode
        rtmBot.send(iT('en.buildingContext.facilitatorSetTopic'), facilitatorDM)
        Object.keys(dmIds).forEach(function (userId) {
          rtmBot.send(iT('en.buildingContext.participantSetTopic', {
      ...
      if (existingTopic) {
        ...
        addTopicToMap(id, topic, tokens[user], function (err, t) {
          if (err) {
            rtmBot.send('There was an error creating that topic', facilitatorDM)
            return
          }
          configuration.linkedMap.topics.push(t)
          complete(t)
  ...
  function setMetacode (name) {
    ...
    if (metacode) {
      rtmBot.send(iT('en.buildingContext.facilitatorSetMetacode', metacode), facilitatorDM)
      selectedMetacode = metacode
      Object.keys(dmIds).forEach(function (userId) {
        rtmBot.send(iT('en.buildingContext.participantSetMetacode', metacode), dmIds[userId])
      })
    } else {
      rtmBot.send(iT('en.buildingContext.facilitatorNoMetacode'), facilitatorDM)
    }
  }
  function unsetTopic () {
    focalTopic = null
    rtmBot.send(iT('en.buildingContext.facilitatorUnsetTopic'), facilitatorDM)
    Object.keys(dmIds).forEach(function (userId) {
      rtmBot.send(iT('en.buildingContext.participantUnsetTopic'), dmIds[userId])
    })
  }
  ...
  rtmBot.send(iT('en.buildingContext.facilitatorCommands'), facilitatorDM)
  rtmBot.send(iT('en.buildingContext.facilitatorExplain'), facilitatorDM)

  // offer the initial topic to the participants
  Object.keys(dmIds).forEach(function (userId) {
    rtmBot.send(iT('en.buildingContext.participantInitialTopic', focalTopic), dmIds[userId])
    rtmBot.send(iT('en.buildingContext.participantInitialMetacode', selectedMetacode), dmIds[userId])
  })
